...
**`bin/damagefps.c`** counts X DAMAGE events on the rekordbox window. Damage is the
...
**Harnesses:** `bin/fpsmatrix.sh` (one-variable-at-a-time A/B, with a cleandown and a
config-survived-the-launch check between variants) and `bin/uisoak.sh` (long runs,
trend reporting).
...
`bin/fpsmatrix.sh --variants`, one variable per run, each verified to have survived
the launch:
...
`bin/uisoak.sh`, 20 s samples, a track loaded and playing:
...
address-space growth is i915 GEM*. `bin/uisoak.sh` now samples
...
2. **The whole renderer matrix in §4 was measured with BOTH DECKS EMPTY**, at the
   ...
   ever been tested under the fault.** `bin/fpsmatrix.sh` now loads a track and
...
frame limiter otherwise. `bin/uisoak.sh` requires a track to be *loaded*, and
...
### The controlled pair, with `bin/playkeep.sh` keeping the deck playing
...
27 minutes, GL renderer, deck kept playing by `bin/playkeep.sh` (12 track
