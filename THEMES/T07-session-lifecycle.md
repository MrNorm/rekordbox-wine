...
| `bin/uimatrix.sh:43,94,112` | `pkill -f rekordbox.exe` + `wineserver -k` | the whole Agent tree |
| `bin/rbtrace.sh:22`, `bin/hidhide-test.sh:57` | `wineserver -k` | the whole Agent tree |
...
2. **Require the process to be a Wine process** — `/proc/<pid>/exe` must resolve into
   ...
   `bin/fpsmatrix.sh` died with "Terminated" on its first cleandown.
...
- `bin/rbw`, `bin/uimatrix.sh`, `bin/rbtrace.sh` and `bin/hidhide-test.sh` still call
