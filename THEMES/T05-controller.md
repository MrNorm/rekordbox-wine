...
`upstream/0003-winealsa-signal-event-only-when-a-period-is-free.patch` proposes
...
`upstream/NOTES-iface-format-search.md`. **Do not run that search.**
...
3. `DeviceLogEnable=1` plus a `DeviceLog.conf` and a listener on 127.0.0.1:10001
   (`bin/devicelog.py`) produced **no output** across two launches. Either
...
`bin/hidhide-test.sh` revokes the user ACL on `/dev/hidraw0`, so winebus cannot
...
`bin/usbreset.sh` already refuses to reset the bus under a running rekordbox for
...
`bin/usbwire.sh` (new, validated 2026-08-16) captures the DDJ's URBs via usbmon
...
`bin/authreplay.py` talks to `/dev/snd/midiC1D0` directly: no rekordbox, no
...
Three arms of `bin/authreplay.py`, back to back, same device, same script, one
...
of those 66 bytes leave the wire in a real run?** `bin/authprobe.sh` puts
...
we have not yet named. `bin/authprobe.sh` settles it by counting the bytes at
the wire.
...
`bin/rbbreak.sh` on `djplay::DeviceMidi::openDevice` (0x1423a5210) and the
...
`bin/rbbreak.sh` on `openDevice` (0x1423a5210) and the validation wrapper
...
`bin/rbbreak.sh` on `djplay::MidiOutputHelper::run` (0x1423A9270) and
...
**The consequence for this session's own results.** `bin/authprobe.sh` gates
...
hand": the gate belongs in the launcher, so use `bin/authprobe.sh` and extend it
rather than reaching for a bare `wine` command.

### Phase 22g — THE RACE TABLE WAS MY OWN HARNESS BUG. Withdrawn entirely.

`bin/authprobe.sh` built rekordbox's environment with
...
`STATE.md` as "generic path" contain **40-41 `@Activate` keep-alives each**, and
...
`python3 bin/authreplay.py observe --secs 25` — expect `@AuthReq`. Then run
...
It also explains, at last, why `bin/authreplay.py` never once failed to deliver
...
`bin/authreplay.py`, back to back, same device, same session, **identical
...
power-cycled before retrying. That is worth stating plainly in `PATH-TO-GOLD.md`
