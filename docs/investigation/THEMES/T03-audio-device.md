...
- **winealsa's answers are optimistic.** It opens `plughw:` (ALSA's conversion
  ...
  list. Noted in `docs/PATH-TO-GOLD.md` as a known wrinkle.
...
`upstream/patches/0002-mmdevapi-allow-event-driven-exclusive-streams.patch` drops the
...
`research/probes/playtest.sh` (new) closes the loop: it drags a track onto deck 1, clicks
...
### Before / after, same hardware, `research/probes/playtest.sh`
...
starting a few seconds later". A 60 s `research/probes/playtest.sh` run showed why — the
...
New tool `research/probes/uilag.sh`, written because "the UI is extremely laggy" was a real
...
New instrument `research/probes/stallscope.sh`: MIDI wire counters, both PCM substreams,
...
rekordbox. `research/probes/stallscope.sh` now samples rekordbox's own PipeWire node
...
`research/probes/meterscope.py` samples the toolbar strip through the compositor
...
Two variants, 75 s each, idle, no human, scored by `research/probes/meterscope.py`:
...
`research/probes/threadscope.py` samples all ~200 rekordbox threads every 250 ms — CPU
...
`research/probes/dualsink.py` samples the DDJ substream and the laptop sink's **monitor** in
...
2. **Diff the threads between the two arms**, not between phases of one arm.
   `research/probes/threadscope.py` refuted render-blocking by comparing burst against
...
- **The audio thread being blocked.** Corrected: `research/probes/threaddiff.py` (fixed, see
...
run time from one binary (`research/probes/periodmatrix.sh` runs an arm and scores it):
...
measured with `research/probes/dualsink.py` (DDJ substream and the laptop monitor on one
clock):
