...
`spectacle -a -b -n -o file.png` — the form used by `research/probes/playtest.sh`,
`research/probes/uiprobe.py`, `research/probes/meterscope.py` and everything else in this repo that
...
`research/probes/threaddiff.py` computed CPU as (jiffies at the end - jiffies at the start),
...
distorted. `research/probes/apitally.py` exists and is useful for RARE calls, but any run of
...
- `research/probes/queueburst.py` reported "0 threshold crossings, 0 teardowns" and its
...
This matters because `docs/GOLD-STATUS.md` cited "1.00x" from this instrument as
