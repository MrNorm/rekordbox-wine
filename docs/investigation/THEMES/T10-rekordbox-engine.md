...
| tool | what it does |
|---|---|
| `research/probes/pexref.py` | every RIP-relative reference to a string, plus absolute pointers to it, by arithmetic sweep with numpy (54 MB `.text` in under a second) |
| `research/probes/pexrefva.py` | the same for an arbitrary VA range — "who reads this global" |
| `research/probes/pefunc.py` | exact function bounds from the PE's `.pdata` RUNTIME_FUNCTION table — instant, no analysis needed |
...
`docs/investigation/STATE.md` as an unexplained mystery since phase 24. It is not a stall; it is a
scheduled retry.
...
`research/probes/pexrefva.py` assumes an instruction ends 4 bytes after the RIP
...
`docs/PATH-TO-GOLD.md` with its reversal) so `gdb` could attach to a running
...
`research/probes/queuescope.py` reads the engine's per-device queues through
...
contract. Build it, install it, and re-run `research/probes/queuescope.py` in the same
...
(reversal recorded in `docs/PATH-TO-GOLD.md`). Run `runs/QUEUE/20260819T063207-dblbuf.log`,
...
Extend `research/probes/queuescope.py` with a `--trace` mode that writes every sample
...
`research/probes/queuescope.py --trace`, baseline driver restored, PC MASTER OUT on, 75 s
...
`research/probes/queuescope.py` for 70 s with the callback neutered:
...
### research/probes/timerscope.py — every live JUCE timer, its period, and its owner class
...
`research/probes/queuescope.py` for 100 s with the timeout quadrupled — collapse intervals:
...
`research/probes/threadpulse.py` (next): sample every thread's `utime+stime` and run state
...
`research/probes/threadpulse.py` (new) samples every thread's `utime+stime` and run state
...
do not exist. `research/probes/queuescope.py` was also fixed here: it required a non-zero
...
`research/probes/alsapulse.py --trace`, 80 s, 3955 samples at 49 Hz:
...
`research/probes/queuescope.py --trace`.
...
out to date: every JUCE timer in the process (phase 13, `research/probes/timerscope.py`
...
`research/probes/queuescope.py` computed `spread = max(depths) - min(depths)` across **every
...
`research/probes/threadpulse.py` extended to nanosecond CPU and scheduling counts from
...
Trace the *fine* structure. `research/probes/queuescope.py --trace` at 80 Hz cannot resolve
...
`research/probes/queueburst.py` (new) samples both queue depths as fast as `/proc/<pid>/mem`
...
2. **Install `perf`** (needs root; `kernel.perf_event_paranoid` is 2 and would
   ...
   window from `research/probes/queueburst.py`. Nothing wakes *often*, but something must
...
`perf` installed (recorded in `docs/PATH-TO-GOLD.md` with reversals, along with
...
three cycles, align it to the onset times from `research/probes/queueburst.py` (both use
...
`research/probes/threadpulse.py` looked for this at 80 Hz and found nothing; `sched_switch`
...
104 MB), aligned to `research/probes/queueburst.py`'s onsets — both now report
...
`research/probes/queuephase.py` was written to test a beat hypothesis: two devices on
...
embedded frame counts, or for a pointer to a WASAPI buffer. `research/probes/queuescope.py`
...
`CLOCK_MONOTONIC`, the same clock `research/probes/queueburst.py` prints, so a gap lines up
...
Four complete fault cycles (`research/probes/queueburst.py`: teardowns at MONO
...
`research/probes/queueburst.py` (teardowns at MONO 376107.52, 376123.40, 376139.24,
...
`research/probes/queuescope.py` can supply a live entry address to anchor the search, and
the `.pdata` + RTTI tooling in `research/probes/pefunc.py` / `research/probes/pexrefva.py` is already set
...
`research/probes/poolcond.py` (new) polls all four fields plus the queue depth. Over 70 s of
...
`research/probes/popcount.py` (new) samples the head pointer at `device + 0x68 + slot*8` and
...
1. `research/probes/popcount.py` already gives the exact churn windows.
2. Sample `/proc/<pid>/task/*/schedstat` and `/wchan` through a full cycle
   (`research/probes/threadpulse.py` does both) and find the thread whose `run_ns` is flat
...
`research/probes/enginethread.py` (new) samples the thread set **continuously** and
...
2. Or catch it in the act: with `ptrace_scope` at 0, read
   ...
   installed; its sysctls need re-enabling as recorded in `docs/PATH-TO-GOLD.md`.
...
(T+1.000, and `research/probes/queueburst.py` locates T to the millisecond), so:
...
Re-run after the reboot with the lesson from `docs/investigation/THEMES/T00`: **six named events, in
...
same one `perf -k CLOCK_MONOTONIC` and `research/probes/queueburst.py` use, so windows line
...
`research/probes/peimports.py` (names every IAT slot -- `objdump -x` prints hint numbers for
this binary, so `call *0x1433747c8` was anonymous until now; it is
`TryEnterCriticalSection`), `research/probes/pecslock.py` (every `lea +off(%reg)` paired with
a CRITICAL_SECTION call, for "who locks this member"), `research/probes/enginefind.py`
...
`upstream/patches/0002-mmdevapi-exclusive-event-streams.patch`, written by this project
...
shipping behaviour exactly, so the arm is one environment variable:
    ...
    research/probes/barrierscope.sh 40
...
`research/probes/barrierscope.sh 40` on the same instance:
...
For `docs/PACKAGE.md` and the run-and-play script this is two lines: ship the mmdevapi
...
`research/probes/queuescope.py` over 150 s says exactly why:
...
it is measured wrong.** `research/probes/barrierscope.sh 25` against a healthy arm
