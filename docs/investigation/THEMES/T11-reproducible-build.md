...
pristine Wine tarball plus `upstream/patches/0001..0009`. Those are two different
...
`research/retired/install-system-wine-patches.sh` knew only about `winealsa.so` and now
...
source splice `upstream/patches/rbw-usbhcd.c`. It exposes `\\.\HCDn`. Without it,
...
`docs/PATH-TO-GOLD.md` step 3c already called it *mandatory for the controller*. It
...
`docs/PACKAGE.md` for what the package installs, [[T00-instrument]] for why a
