...
`docs/investigation/STATE.md`, the last three entries of `docs/investigation/JOURNAL.md`, and
`docs/investigation/THEMES/T03-audio-device.md` from phase 25 onward.

## Where this stands

The controller is **done** (`upstream/patches/0004-winealsa-midi.patch`; a SysEx split
...
- **T09, new: one exclusive `hw:` open can delete a device from PipeWire for the
  ...
  `research/probes/pwclash.sh`. This machine had **no audio sinks at all** for the second
  half of the previous session because of it, which voids the runs that
  concluded RBW-RING "broke WASAPI". Report drafted:
  `upstream/reports/wireplumber-alsa-node-error-handler.md`. The launcher now detects
  and repairs it.
...
proof), `bin/loadtime.sh` (track-load timing), `research/probes/playkeep.sh` (keep a deck
playing through a soak), `research/probes/dualsink.py` (DDJ substream + PC monitor on one
clock), `research/probes/threaddiff.py` (per-thread CPU/wchan, comparable across arms),
`research/probes/pwclash.sh` (the PipeWire coexistence reproducer), `research/probes/snddev.sh` (which
sound devices the prefix opens), `research/probes/apitally.py` (Wine debug-channel tallies —
