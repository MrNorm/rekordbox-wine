...
**This is the to-do list, not the status page.** `docs/GOLD-STATUS.md` records what
works and the evidence for it; `docs/investigation/STATE.md` says what to do next session. This
...
**Published 2026-08-20: https://github.com/MrNorm/rekordbox-wine** — a single
sanitised commit, no PII, LICENSE in place. All three packaging definitions now
point at it. What remains is an AUR submission: tag a release, replace
`sha256sums=('SKIP')` with the real hash, and push to `aur.archlinux.org`.
...
`upstream/reports/appdb-rekordbox-7217.md` line 52 states *"Bug report and patch filed
...
Ten patches plus a source splice, none sent. `upstream/patches/0001` (dxgi
...
Now defect 12 in `docs/GOLD-STATUS.md`, with a report drafted at
`upstream/reports/bugzilla-mmdevapi-session-notification.md`. Still **unfiled** (A2),
...
`upstream/patches/expected-divergence.txt`. Wired into `docs/REGRESSION.md` as a release
...
deleting one directory. See `docs/investigation/THEMES/T13`.
