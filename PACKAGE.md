...
This replaces the scattered patch list that had accumulated in `PATH-TO-GOLD.md`
and `STATE.md`. It is the authoritative set. Every entry here is **verified
loaded and load-bearing**; anything that could be removed without breaking a
measured behaviour has been removed.

## The Wine patch series — `upstream/0001..0009`, plus the `wineusb` splice
...
source file: `upstream/rbw-usbhcd.c`, applied by `bin/build-wineusb-hcd.sh`.
...
| the `RBW-WIRE` / `RBW-RAW` hex probes | debug-only, and both truncated (64 bytes out, 32 in with no marker) which actively misled the investigation. Superseded by `bin/usbwire.sh`, which reads the USB wire and is external to the shipped code. |
...
## How to verify an install
    ...
    ./bin/authprobe.sh --runs 3       # must report "@AuthEnd RECEIVED" every time
...
| setting | value | why |
|---|---|---|
| `WasapiPolling` | `1` | Without it PC MASTER OUT runs at **0.05x** of real time and both audio streams are torn down and rebuilt every 15.9 s. `THEMES/T10` phases 34-38. |
| `AudioBufferSize` | `512` (floor) | At 256 a teardown still arrives about once every three minutes, because no rekordbox thread gets real-time priority under Wine. `THEMES/T10` phases 40-42. |
...
| piece | what |
|---|---|
| `upstream/0008-setupapi-physical-device-object-name.patch` | `SPDRP_PHYSICAL_DEVICE_OBJECT_NAME` was a NULL placeholder; without it rekordbox can never match a device to a drive letter. System-wide `setupapi.dll`; verify with `strings … \| grep -c PhysicalDeviceObjectName` (expect 2). |
| `upstream/0006` + `0007` | the drive must report `DRIVE_REMOVABLE`, and the storage descriptor must not claim every device is a fixed SCSI disk. |
| read access to `/dev/sdX1` | otherwise Wine reports a FAT32 stick as NTFS. Ship a udev rule. |
| `upstream/0009-mountmgr-volume-devnodes.patch` | writes the `STORAGE\Volume` devnode from inside `mountmgr.sys`, on every `set_volume_info()`, and removes it on `delete_dos_device()`. Without a devnode, SetupAPI enumerates no volumes at all and rekordbox's USB panel is empty however healthy the mount is — it intersects the SetupAPI volume list with `GetDriveType`. This **retires `bin/usbdevnode.sh`**, which wrote the same key from outside keyed on a `\Device\HarddiskN` name that is assigned in plug order and so was per-stick and per-session. |
...
|---|---|---|
| `wineusb.sys` (PE driver) | `upstream/rbw-usbhcd.c`, **spliced** into `wineusb.c` by `bin/build-wineusb-hcd.sh` | creates the `\\.\HCDn` device objects and answers the USB hub IOCTLs |
| `wineusb.so` (unix side) | `upstream/0010-wineusb-hcd-unixlib.patch` | reads the kernel's own view of the bus out of `/sys/bus/usb/devices`, so the descriptors an application reads back are the device's real ones |
...
matching the *installed* Wine, applies `upstream/0001..0009` in order, configures
...
`sudo bin/install-system-wine-patches.sh`, which backs up each stock file and
...
markers absent. See `THEMES/T11-reproducible-build.md`.
