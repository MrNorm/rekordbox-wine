...
**The AUR cleanbuild is verified: a build from `upstream/0001..0009` and a
...
write-up in `THEMES/T11-reproducible-build.md`.
...
`GOLD-STATUS.md` cited **1.00x** from `bin/soak.sh`. That instrument samples
...
patch: `wineusb.sys`/`wineusb.so`, spliced in from `upstream/rbw-usbhcd.c`,
which exposes `\\.\HCDn`. `PATH-TO-GOLD.md` step 3c already called it
...
and every measurement are in `THEMES/T10` phases 33-42. The fix is two rekordbox
...
* **Hardware execute breakpoints.** `perf record -e mem:<code addr>:x -p <pid>`
  ...
  register limit). `bin/barrierscope.sh` wraps the four that matter.

New instruments this session: `bin/barrierscope.sh`, `bin/soak.sh`,
`bin/arm.sh`, `bin/teardownmark.py`, `bin/enginefind.py`, `bin/peimports.py`
(names every IAT slot — `call *0x1433747c8` was anonymous until now; it is
`TryEnterCriticalSection`), `bin/pecslock.py`.
...
move and dismissed the menu **11 ms after mapping it**.
    ...
    upstream/0005-winex11-popup-not-managed.patch      marker RBW-POPUP
...
**What made it work** (all in `PATH-TO-GOLD.md` steps 8-9, with reversals):

    upstream/0006  mountmgr: a removable UDisks drive is removable
    upstream/0007  mountmgr: stop faking RemovableMedia and BusType
    upstream/0008  setupapi: SPDRP_PHYSICAL_DEVICE_OBJECT_NAME readable
    ACL on /dev/sdX1 — without it a FAT32 stick reads as NTFS
    bin/usbdevnode.sh — writes the volume devnode  (STOPGAP)
...
**Move the devnode into `mountmgr.sys`.** `bin/usbdevnode.sh` is a stopgap and is
...
when the volume goes. The key discovery that makes this cheap is in `THEMES/T02`:
...
two dead ends are already ruled out (see `THEMES/T02`): the Devices-only code in
...
Draft and evidence: `upstream/NOTES-mmdevapi-buffer-widening.md`.
...
wheels work, on plain launches, via `upstream/0004-winealsa-midi.patch`. See
`THEMES/T05` phases 23-24. Do not reopen without new evidence.

**Environment: CLEANED.** `PACKAGE.md` is the authoritative install list.
...
limiter. With `bin/playkeep.sh` keeping the deck playing:
...
`bin/pwclash.sh` (`20260817T165505`): one direct `hw:` open makes PipeWire's own
...
dead after a set. See `THEMES/T09-pipewire-coexistence.md`.
...
follow from it are in `THEMES/T00-instrument.md`.
...
**Controlled proof, Wine-free, one variable** (`bin/authreplay.py`, back to back,
...
applications write to **rawmidi**, which is why `bin/authreplay.py` never once
...
Ranked options in `THEMES/T05-controller.md` phase 23; the preferred one is a
...
`bin/usbwire.sh` shows whether the 66 bytes leave as one URB or two.
...
- **Instrument warnings.** The 0xFE wire check passes on a device that is merely
  ...
  reported). `bin/authprobe.sh` renamed the MIDI port on every run until
...
**Read `THEMES/T05-controller.md` phases 20-21c before touching this.** New
instruments: `bin/usbwire.sh` + `bin/usbwire.py` (USB wire capture with URB
completion codes), `bin/authreplay.py` (drives the handshake from pure Linux),
`bin/authprobe.sh` (full-stack, both layers correlated).
...
`./bin/authprobe.sh --secs 55 --label wire3 --runs 3`. Outcomes:
...
measured rather than described. **Read `THEMES/T07-session-lifecycle.md` and
`THEMES/T08-frame-rate.md` before touching this.**
...
`bin/damagefps.c` counts X DAMAGE events — no pixels, so the black-XWayland-capture
...
`bin/uisoak.sh`, track loaded: **55.4 → 36.8 fps over nine minutes**, worst gap
...
2. **Re-run the renderer matrix with a track loaded** — see the correction below; it
   has never been run under the fault. `bin/fpsmatrix.sh` now loads a track itself.
...
Patch `upstream/0006-winealsa-enumerate-only-subscribable-midi-ports.patch`,
...
Do NOT run the format search in upstream/NOTES-iface-format-search.md.
...
- `/usr/lib/wine/x86_64-unix/winealsa.so` now carries patches 0003, 0004 **and
  ...
  `sudo bin/install-system-wine-patches.sh --revert`.
...
Patched (`upstream/0002-...patch`), the Sample Rate dropdown populates and
...
**Packaging deliverable:** `PATH-TO-GOLD.md` — the ordered recipe with the reason
...
watching `bin/uimatrix.sh` reported the first two variants (`patched-d2d`,
...
`bin/uimatrix.sh`, with the readiness gate, run `20260813T0914*`:
...
### Tooling built for this — `bin/uiprobe.py` + `bin/uimatrix.sh`
...
`bin/uimatrix.sh` runs `scenarios/main-ui.json` across renderer/patch variants
...
- **The instrument works.** `rbw selftest` returns `window-ok` for notepad with
  ...
  found and fixed on the way — see `THEMES/T00-instrument.md`. Two of them (X11
...
- `THEMES/T00-instrument.md` — RESOLVED. Re-verify after any Wine/KDE/IM/tesseract upgrade.
- `THEMES/T01-first-window.md` — **RESOLVED 2026-08-13**, root cause proven and
  patched. Reopen only if a JUCE app still freezes with the patch applied.
- `THEMES/T02-usb-export.md` — **OPEN and now the biggest gap.** Stick is
  formatted and mapped as E: removable; export itself has never been run.
- `THEMES/T03-audio-device.md` — **OPEN, and now well posed.** Playback works
  ...
  phase 25 onward.
- `THEMES/T09-pipewire-coexistence.md` — **NEW, OPEN.** One exclusive `hw:` open
...
- `THEMES/T04-menus.md` — **OPEN.** File menu never opens (0/5); creates no
  window at all, so it is not the repaint bug.
- `THEMES/T05-controller.md` — **OPEN, critical path.** Binds only under a renamed
...
- `THEMES/T07-session-lifecycle.md` — **RESOLVED 2026-08-14.** Leaked sessions and
  the lost ntsync. Reopen if orphans reappear after a launch through
  `bin/rekordbox-wine`.
- `THEMES/T08-frame-rate.md` — **OPEN, and it is the current lag work.** Frame rate
...
**(0b) NEXT ACTION — build and test `upstream/0003-winealsa-signal-event-only-when-a-period-is-free.patch`.**
...
written (`upstream/bugzilla-dxgi-waitforvblank.md`,
`upstream/0001-dxgi-implement-WaitForVBlank.patch`). Two things must be done by
a human first:
...
**(4) AppDB submissions — drafted, unfiled** (`upstream/appdb-rekordbox-7217.md`):
