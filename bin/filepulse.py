...
WHY THIS EXISTS. THEMES/T10 phase 11 showed the audio teardown is triggered by a
...
`bin/queuescope.py --trace` records in its header, so the two logs can be laid
side by side and each write matched against each queue excursion.

Usage: bin/filepulse.py <dir> [seconds]
"""
import sys, os, time
