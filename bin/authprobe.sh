...
one_run() {
    ...
    if ! "$REPO/bin/usbwire.sh" check | tee "$dir/wirecheck-before.txt"; then
    ...
    "$REPO/bin/usbwire.sh" start "$LABEL-r$n" >"$dir/capture.txt" 2>&1
    ...
    "$REPO/bin/usbwire.sh" stop > "$dir/wire.txt" 2>&1
    ...
    "$REPO/bin/usbwire.sh" check > "$dir/wirecheck-after.txt" 2>&1
