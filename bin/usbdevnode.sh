...
# enumerations (THEMES/T02): SetupAPI's GUID_DEVCLASS_VOLUME devices on one
...
#
# SUPERSEDED 2026-08-19 by upstream/0009-mountmgr-volume-devnodes.patch, which
...
# `bin/usbdevnode.sh --remove` and check whether mountmgr recreates the entry
# under Enum\STORAGE\Volume\WineVolume<letter> by itself. If it does not, 0009
# is not installed.
#
# Needs the patched setupapi (upstream/0008) or the property read fails anyway.
#
# Usage: bin/usbdevnode.sh [--remove]
set -uo pipefail
