...
**Tier 0 runs first or everything downstream is worthless.** `THEMES/T00-instrument.md`
...
`20260812T194755-control-ctl-notepad`; table in `THEMES/T00-instrument.md`):
...
### 0.3 — capture path is spectacle, not an X11 grabber
    ...
    grep -n "spectacle" bin/rbw bin/uiprobe.py bin/uiassert.py bin/menuprobe.sh
...
### 0.4 — `uiassert` refuses to grade an ungradeable screenshot
    ...
    ./bin/uiassert.py --shot runs/CALIB/main.png            # must exit 0
    ./bin/uiassert.py --shot upstream/devtree-output-stock.txt ; echo $?   # must exit 2
...
The live calibration gate is in `bin/uiassert.py:161` — if the always-enabled
...
never parsed. `upstream/NOTES-iface-format-search.md` must not be run.
...
### 2.1 — `uiassert` baseline — the four binding tells
    ...
    ./bin/uiassert.py --capture --expect baseline
    ./bin/uiassert.py --shot runs/CALIB/main.png --expect baseline    # offline
...
### 2.3 — `uimatrix` — patched vs stock vs software renderer
    ...
    ./bin/uimatrix.sh                          # all three variants
    ./bin/uimatrix.sh patched-d2d              # one variant
...
### 2.4 — `menuprobe` — do the top-level menus open
    ...
    ./bin/menuprobe.sh runs/menu-$(date +%s) 5
...
### 3.2 — `devwatch` — passive timeline of the binding path
    ...
    ./bin/devwatch.sh 1800 > runs/_devwatch/session-$(date +%Y%m%dT%H%M%S).tsv
...
### 3.4 — `midiInOpen` trace — does rekordbox even try
    ...
    ./bin/rbtrace.sh midiopen '+midi,+winmm' 100
...
### 3.6 — `devicelog` — rekordbox's own words
    ...
    ./bin/devicelog.py runs/DEVICELOG/devicelog.txt
...
have caught it. Sources are `JOURNAL.md`, `THEMES/*`, and `git log`.
...
| R2 | **A prefix update restored a stock DLL over the patched one.** Cited in `PATH-TO-GOLD.md` and the `bin/rekordbox-wine` header as one of the three silent reverts that motivated `--check`. | Application behaved as unpatched; no error anywhere. | **1.9** byte-compare against the artifact (`cmp`), not "is a file present" |
...
8. **Add it here**, in the tier table and in full, with its command, its recorded
   ...
   add the citation to the relevant `THEMES/T0x.md`.
...
| **`scenarios/regions.json` `anchors` reference `bin/uidrive.sh`, which does not exist.** | **GAP** |
...
Run `bin/usbreset.sh` **before any controller measurement**. The controller
...
obviously wrong.
    ...
    bin/usbreset.sh --check    report device/ALSA/hidraw/ACL state, change nothing
    bin/usbreset.sh            deauthorize + reauthorize, wait for re-enumeration
...
## Audio — PC MASTER OUT (added 2026-08-19, `THEMES/T10` phases 33-45)
...
| check | command | pass |
|---|---|---|
| the start-up rendezvous | `bin/barrierscope.sh 40` | **1** barrier opening, ~3.5 s after the stream starts, then none; both callback threads within 1% of each other |
...
| the queues | `bin/queuescope.py <pid> 150` | `trim_count resets (= announces): 0` |
| real audio out | `parec --device=<sink>.monitor …` for 10 s | continuous, non-silent, no per-second gaps |
| performance load | `bin/loadplay.sh 3` + `bin/loaddeck2.sh 4`, both playing | 0 teardowns in 120 s |
...
## USB export (added 2026-08-19, `THEMES/T02`) — **Tier 2**

Needs a FAT32 stick attached. Ten minutes, mostly unattended.

    bin/usbdevnode.sh                       # after plugging the stick in
...
Finally, install them and prove behaviour, not just strings:
    ...
    sudo bin/install-system-wine-patches.sh
...
"pristine + `upstream/0*.patch`" and fails on any file not listed in
`upstream/expected-divergence.txt`. Every entry in that allowlist must be
