...
**One page, then the evidence.** Read `STATE.md` for what to do next and
`PATH-TO-GOLD.md` for how to build the thing. This document answers a different
...
| removable-drive detection | **FIXED (today)** — `E: REMOVABLE / REKORDBOX / FAT32`, was `FIXED / NTFS` | `upstream/0006` + device-node access |
| storage descriptor honesty | **FIXED (today)** — removable media and bus type were faked for every device | `upstream/0007` |
| volume devnode for SetupAPI | **FIXED** — now written by `mountmgr.sys` itself; `bin/usbdevnode.sh` retired | `upstream/0009`, T02 |
...
|---|---|---|---|
| 1 | `dxgi`: `IDXGIOutput::WaitForVBlank` unimplemented | any JUCE 8 app paints **one frame** and freezes — this is why nobody had ever reported rekordbox 7.2.x reaching its window | **fixed**, `upstream/0001` |
| 2 | `mmdevapi`: event-driven exclusive streams refused outright | empty sample-rate list; no DJ controller usable at all | **fixed**, `upstream/0002` |
| 3 | `winealsa`: exclusive-mode event never signalled on a free period | stream starves and collapses | **fixed**, `upstream/0003` |
| 4 | `winealsa`: SysEx split across two USB transfers | DDJ-400 never authenticates; LEDs flash, no control works | **fixed**, `upstream/0004` |
| 5 | `winex11`: `is_window_managed()` calls a `WS_POPUP\|WS_SYSMENU` window managed | KWin moves JUCE's off-screen drop shadow; JUCE dismisses the menu 11 ms after mapping. **No File menu, no EXPORT mode** | **fixed**, `upstream/0005` |
| 6 | `mountmgr`: a removable UDisks drive with empty `MediaCompatibility` stays `DEVICE_UNKNOWN` | `GetDriveType` never returns `DRIVE_REMOVABLE` for a USB stick | **fixed**, `upstream/0006` |
| 7 | `mountmgr`: `StorageDeviceProperty` is faked — `RemovableMedia = FALSE`, `BusType = BusTypeScsi` for **every** device | applications that filter on removability or bus type see a USB stick as a fixed SCSI disk | **fixed**, `upstream/0007` |
| 8 | `setupapi`: `SPDRP_PHYSICAL_DEVICE_OBJECT_NAME` is a NULL placeholder | an application cannot match an enumerated device to a drive letter, so a USB stick can never be found | **fixed**, `upstream/0008` |
| 9 | `mountmgr` does not write a `GUID_DEVCLASS_VOLUME` devnode | SetupAPI enumerates no volumes at all, so a USB stick can never be found however healthy the mount is | **fixed**, `upstream/0009` |
| 10 | `wineusb`: no `\\.\HCDn` device object exists at all | rekordbox validates a Pioneer controller by walking the USB bus the way `usbview.exe` does; ten `CreateFileW` on `HCD0..9` all fail, so it **destroys the device object it just built** and never opens the controller's MIDI port | **fixed**, `upstream/rbw-usbhcd.c` |
...
`upstream/NOTES-mmdevapi-buffer-widening.md` — *widening a client's buffer is
not invisible to the client.*

## What is left between here and Gold

**The actionable list lives in [`REMAINING-STEPS-TO-GOLD.md`](REMAINING-STEPS-TO-GOLD.md)** — who can do each item and what would count as done. What follows is the reasoning behind it.

### Blocking Gold

1. ~~The volume devnode belongs in `mountmgr.sys`.~~ **Done** —
   `upstream/0009`. mountmgr now writes and removes the `STORAGE\Volume`
   devnode itself, as `set_volume_info()` and `delete_dos_device()` already know
   every field it needs. The stopgap `bin/usbdevnode.sh` wrote that key from
...
6. **The package has never been installed on a SECOND MACHINE.** It has now
   ...
   has followed `PACKAGE.md` end to end anywhere else.
