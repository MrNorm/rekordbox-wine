...
| **desktop keeps its audio devices** | **needs the launcher's repair** — an exclusive `hw:` open can cost PipeWire the device permanently | T09, `bin/pwclash.sh` |
...
Upstream: `upstream/0001-dxgi-implement-WaitForVBlank.patch`, report draft in
`upstream/bugzilla-dxgi-waitforvblank.md`, standalone reproducer
...
Upstream: `upstream/0002-mmdevapi-allow-event-driven-exclusive-streams.patch`.
...
both are system-level rather than prefix-level. See `THEMES/T05-controller.md`.
...
5. **The package has never been installed from scratch.** `packaging/PKGBUILD`
   exists and `PACKAGE.md` is the authoritative list, but nobody has built the
...
| ~~`bin/install-system-wine-patches.sh`~~ | **SUPERSEDED by step 13** — it replaced files owned by the distro's `wine` package. Use `bin/make-private-wine.sh`, which needs no root. |
...
With `bin/playkeep.sh` keeping the deck playing (it checks the deck's own
...
Check it yourself at any time — no rebuild, no root:
    ...
    bin/playkeep.sh 660 &        # keep it playing
    bin/uisoak.sh 10             # fps, jitter and gpu_mb per sample
...
`Rx 63`.
    ...
    sudo bin/install-wineusb-hcd.sh     # replaces both, backups at *.rbw-backup
...
**Reversal:** `sudo bin/install-wineusb-hcd.sh --revert`, then `bin/rbclean.sh`.
...
| change | why | reversal |
|---|---|---|
| `sudo modprobe usbmon` | passive USB capture for `bin/usbwire.sh`; without it the DDJ's URBs are invisible | `sudo rmmod usbmon` |
...
controlled experiment; see `THEMES/T05-controller.md` phase 23b.
...
and the same `bin/build-patched-dlls.sh` / `bin/install-system-wine-patches.sh`
...
the spurious device-change announcement (THEMES/T10 phase 7).
...
Being measured against `bin/queuescope.py`: does the DDJ's exclusive-mode queue
still outgrow the PC endpoint's by 4 buffers (THEMES/T10 phase 9)?
...
**REVERTED 2026-08-19** — the patch was refuted (THEMES/T10 phase 10: it removes
...
(THEMES/T10 phase 21 proved the alarm exists; three candidate constants have
...
timestamp is `CLOCK_MONOTONIC`, matching `bin/queueburst.py`.
...
well as the controller.** Root cause and every measurement: `THEMES/T10` phases
33-42. Short version:
...
measurement: `THEMES/T04`, and the patch is written up for upstream in
`upstream/0005-winex11-popup-not-managed.patch`.
...
**8a. `upstream/0006-mountmgr-removable-unknown-media.patch`.** Wine's UDisks
...
`SPDRP_PHYSICAL_DEVICE_OBJECT_NAME == QueryDosDeviceW(letter)` (`THEMES/T02`).
Two pieces:

**9a. `upstream/0008-setupapi-physical-device-object-name.patch`.**
...
**9b. `upstream/0009-mountmgr-volume-devnodes.patch`.** `mountmgr.sys` now
...
`bin/usbdevnode.sh` is **superseded** by this and kept only as a fallback for a
Wine without 0009, and as a diagnostic: run `bin/usbdevnode.sh --remove` and
...
only thing that will say so.
    ...
    change:    sudo bin/install-system-wine-patches.sh
    ...
    reversal:  sudo bin/install-system-wine-patches.sh --revert
...
`bin/usbdevnode.sh` is retired by `upstream/0009` and is no longer part of any
...
## Step 11 — the package installed as a package — 2026-08-20
    ...
    reversal:  sudo pacman -R rekordbox-wine
               (this does NOT revert the system Wine libraries — the package
                ...
                bin/install-wineusb-hcd.sh --revert.)
...
makes safe to implement. See `THEMES/T12`.
...
before:
    ...
    sudo bin/install-wineusb-hcd.sh --revert
...
marker check still passes. Measured. See `THEMES/T13`.
